...
import { ReplaySubject, Subject, takeUntil, Observable, finalize } from 'rxjs';
...
export class SingleSelectComponent {
  static readonly COMPONENT_NAME = 'SingleSelectComponent';
  ...
  ngOnInit() {
    ...
    // this.listCtrl.setValue(this.data[1]);
  ...
  private getData(search: string | null = null) {
    if (this.config?.uri) {
      let isValid = true;
  ...
  setCustomClass() {
    ...
    const panelClass = `tis-dropdown-panel tis-client-side-single-select-panel ${this.panelClass}`;
