...
export class SelectMultipleComponent {
    static readonly COMPONENT_NAME = 'SelectMultipleComponent';
    ...
    writeValue(value: any) {
        if (!this.isReset) {
            ...
            if (value?.length) {
                this.getData(null, false, value, false)
            }
    ...
    onInputChange(value: any) {
        ...
        if (value?.length > 0 && value != '*') {
            value?.forEach((v: any) => {
                if (this.initialData?.findIndex(d => d[this.valueKey] == v) == -1) {
                    let selectedData = this.data?.filter(d => d && Object.keys(d).length > 0).find(obj => obj[this.valueKey] == v);
    ...
    private getData(search: string | null = null, isReset: boolean = false, includedIds: any[] = [], initial: boolean = false) {
            ...
            if (isValid) {
                ...
                this.subscription = this.fetchData(this.config.method ?? 'GET', this.config.uri, this.config.limit ?? 20, search, filter ?? {})
                    .pipe(
                        ...
                        let fetchedData = this.getNestedValue(r, this.config?.dataValueKey ?? 'data');
                        let tempData: any[] = fetchedData ?? [];

                        tempData?.forEach(d => {
                            if (this.data?.findIndex(dd => dd[this.valueKey] == d[this.valueKey]) === -1) {
                                this.data.push(d);
                            }
                        });
