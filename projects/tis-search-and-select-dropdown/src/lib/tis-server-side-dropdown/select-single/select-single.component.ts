...
export class SelectSingleComponent {
  ...
  ngOnInit() {
    ...
    this.listCtrl.valueChanges
      ...
      .subscribe((value) => {
        ...
        if (value && value != '' && this.data?.filter(d => d && Object.keys(d).length > 0)?.findIndex(d => d[this.valueKey] == value) == -1) {
  ...
  onInputChange(value: any) {
    if (value && value != '') {
      if (this.initialData?.filter(d => d && Object.keys(d).length > 0).findIndex(d => d[this.valueKey] == value) == -1) {
        let selectedData = this.data?.filter(d => d && Object.keys(d).length > 0).find(obj => obj[this.valueKey] == value);
        this.initialData = [selectedData, ...this.initialData];
      }
    }
    this.onChange(value);
    this.change.emit(value)
    this.initialDataChange.emit(this.initialData);
  }
  ...
  private getData(search: string | null = null, includedIds: any[] = []) {
    if (this.loading && this.subscription) {
    ...
    if (this.config?.uri) {
      ...
      if (isValid) {
        ...
        this.subscription = this.fetchData(this.config.method ?? 'GET', this.config.uri, this.config.limit ?? 20, search, filter ?? {})
          .pipe(
            finalize(() => {
              this.loading = false;
            ...
            if (search == null) {
              ...
              this.initialDataChange.emit(this.initialData);
              if (this.config?.setFirstOption && Array.isArray(this.data) && this.data?.length) {
                if (this.config?.ifLengthOnlyOne === true) {
                  if (this.data?.length == 1) {
                    this.writeValue(this.data[0][this.valueKey]);
                    this.onInputChange(this.data[0][this.valueKey]);
                  }
                }
                else {
                  this.writeValue(this.data[0][this.valueKey]);
